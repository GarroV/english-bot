...
> Всё на **личном** аккаунте `vasiliy.garro@gmail.com`. Supabase-проект — **`btlglelwxazdxfqdmcti` (English_bot)**. ⚠️ В том же аккаунте есть второй проект `vbqglndbxkpmreccpqmr` («GarroV's Project») — **это НЕ он**; всегда сверяй `--project-ref` в каждой команде.

**Полная карта — что где лежит, аккаунты, деплой всех компонентов, чек-лист «чтобы не объебаться», грабли — [ARCHITECTURE.md](ARCHITECTURE.md) (единый источник).**
...
| Файл | Содержание |
|------|-----------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | **Единый источник:** инфраструктура, аккаунты, деплой, чек-лист, грабли |
| [BOT.md](BOT.md) | Архитектура бота, state machine, флоу генерации |
