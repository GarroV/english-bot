...
- `20260708220000_folio_workspaces_generation_quota.sql` — **квота генераций (#75)** (additive): колонка `folio_workspaces.generation_quota int` (NULL = безлимит). Остаток считается на чтении: quota − count(`eb_llm_usage` c `action='module'` по владельцу: бот `ref_id=telegram_id`, folio `ref_id=workspace_id`); канон расчёта — `supabase/functions/_shared/quota.ts`, enforcement — бот `generateAndSend` + `folio-generate` (402 `quota_exceeded`), выдача — админка («+ генерации» / «Безлимит»).
