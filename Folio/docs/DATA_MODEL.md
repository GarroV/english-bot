...
- `20260709140000_folio_hw_inline_answers.sql` — **инлайн-ответы (#56)** (additive): `folio_homework_assignments.inline_answers jsonb` — ответы ученика прямо в пропусках (`___`) текста задания; ключ = порядковый номер пропуска (или `free` для заданий без пропусков). Пишет только `saveInlineAnswers` (token-скоуп + статус-гейт). ⚠️ Контент идёт из шаблона без снапшота: правка шаблона после выдачи может сдвинуть номера пропусков у уже начатых сдач.
- `20260709120000_generation_quota_default_150.sql` — **дефолт месячного лимита = 150** (решение владельца): `generation_quota default 150` для новых воркспейсов + выставлен действующим репетиторам (UPDATE с WHERE; супер-админ не трогался).
- `20260709000000_folio_feedback.sql` — **секция фидбека (#67)** (additive): таблица `folio_feedback` (workspace_id, user_id, category bug/idea/other, message ≤2000). RLS включён без политик — пишет только service-role (Edge Function `folio-feedback` после секрет-гейта; она же шлёт копию владельцу в Telegram). Анти-спам: 1 отзыв/мин на пользователя (считается по индексу user_id+created_at).
- `20260708220000_folio_workspaces_generation_quota.sql` — **квота генераций (#75)** (additive): колонка `folio_workspaces.generation_quota int` (NULL = безлимит). ⚠️ С 2026-07-09 семантика **МЕСЯЧНАЯ**: значение = лимит генераций в месяц, used считается только за текущий месяц по МСК (неиспользованное не переносится, окно само обнуляется 1-го — кронов нет). Остаток на чтении: quota − count(`eb_llm_usage` c `action='module'` за месяц по владельцу: бот `ref_id=telegram_id`, folio `ref_id=workspace_id`); канон расчёта — `supabase/functions/_shared/quota.ts`, enforcement — бот `generateAndSend` + `folio-generate` (402 `quota_exceeded`), выдача — админка («+ генерации» / «Безлимит»).
