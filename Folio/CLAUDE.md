...
> Дизайн вторичен. Красивый UI при кривом фактическом выводе (PDF с квадратами, задание не по делу, битый контент) = бесполезный сервис. Суть — корректный учебный контент.

**Зелёные `npm run build` / deploy / HTTP 200 ≠ «работает».** Задача не закрыта, пока не проверен **фактический вывод глазами на реальном содержимом**:
- Тронул генерацию (`folio-generate`, общий `_shared/generate.ts`) → сгенерировать образец и прочитать его.
- Тронул PDF (`folio-homework-pdf`, общий `english-bot/lib/pdf.ts`) → отрендерить PDF с кириллицей и посмотреть (нет квадратов/обрезки).
- Тронул общий код бота (`english-bot/lib/*`, `_shared/*`) → передеплоить **ВСЕ** потребители (english-bot, folio-generate, folio-homework-pdf, folio-feedback) и смокнуть вывод каждой. Иначе stale-баг в проде.
- `curl → 200` контентом не считается. Не проверить headless → сказать прямо и попросить владельца глянуть.

Полная версия — в корневом `../CLAUDE.md`.
