import { sendMessage, removeKeyboard } from "../lib/telegram.ts";
import {
  isAllowed,
  registerUser,
  setSession,
...
export async function handleStart(message: TgMessage): Promise<void> {
  ...
  if (id === ADMIN_ID) {
    ...
    await sendMessage(chatId, `Добро пожаловать! ${WELCOME}`, removeKeyboard());
    return;
  }
  ...
  if (await isAllowed(id)) {
    await setSession(id, "WAITING_REQUEST");
    await sendMessage(chatId, WELCOME, removeKeyboard());
    return;
  }
...
export async function handleHelp(message: TgMessage): Promise<void> {
  await sendMessage(message.chat.id, HELP, removeKeyboard());
}
...
export async function handleNew(message: TgMessage): Promise<void> {
  ...
  await sendMessage(message.chat.id, WELCOME, removeKeyboard());
}
...
export async function handleInviteCode(message: TgMessage): Promise<void> {
  ...
  await setSession(id, "WAITING_REQUEST");
  await sendMessage(chatId, `Доступ открыт! ${WELCOME}`, removeKeyboard());
}
